...
DictReader::DictReader(const uint8_t* data, int32_t dataLen)
    : _data(data), _dataLen(dataLen), _indexOff(0), _wordCount(0)
{
    ...
    _indexOff  = (int32_t)ReadU32LE(data, 16);
...
std::vector<uint8_t> DictReader::Search(const std::string& word) const {
    ...
    while (lo <= hi) {
        int32_t mid = (lo + hi) >> 1;
        int32_t off = (int32_t)ReadU32LE(_data, _indexOff + mid * 4);
    ...
    std::function<void(const std::string&, const std::vector<uint8_t>&)> callback) const
{
    for (int i = 0; i < _wordCount; i++) {
        int32_t off = (int32_t)ReadU32LE(_data, _indexOff + i * 4);
        int32_t dLen = _data[off];
